...
void createTextureImageView(Vulkan *vulkan) {
    vulkan->textureImageView = createImageView(
        vulkan->device, vulkan->textureImage, VK_FORMAT_R8G8B8A8_SRGB,
...
void generateMipmaps(Vulkan *vulkan, VkImage image, VkFormat imageFormat,
    ...
    vkGetPhysicalDeviceFormatProperties(vulkan->physicalDevice, imageFormat,
                                        &formatProperties);
...
VkSampleCountFlagBits getMaxUsableSampleCount(Vulkan *vulkan) {
    ...
    vkGetPhysicalDeviceProperties(vulkan->physicalDevice,
